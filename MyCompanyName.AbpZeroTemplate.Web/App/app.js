...
appModule.config([
    ...
    function ($stateProvider, $urlRouterProvider, $qProvider) {
        ...
        //BaseContent routes
        $stateProvider.state('basecontent', {
            'abstract': true,
            url: '/basecontent',
            template: '<div ui-view class="fade-in-up"></div>'
        });

        $stateProvider.state('basecontent.suppliermanage', {
            url: '/suppliermanage',
            templateUrl: '~/App/basecontent/views/suppliermanage/index.cshtml'
        });
        if (abp.auth.hasPermission('Pages.BaseContent.SupplierManage')) {
            $urlRouterProvider.otherwise("/basecontent/suppliermanage"); //Entrance page for a tenant
